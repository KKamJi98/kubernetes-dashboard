...
from kubernetes_dashboard.kube_client import (api_for, is_running_in_kubernetes,
...
class TestKubeClient(unittest.TestCase):
    ...
    @patch("kubernetes_dashboard.kube_client.load_kubeconfig_from_secret")
    @patch("kubernetes_dashboard.kube_client.config")
    def test_api_for_k8s_env_with_secret(self, mock_config, mock_load_secret, mock_is_k8s):
    ...
    @patch("kubernetes_dashboard.kube_client.load_kubeconfig_from_secret")
    @patch("kubernetes_dashboard.kube_client.config")
    def test_api_for_k8s_env_without_secret(self, mock_config, mock_load_secret, mock_is_k8s):
    ...
    def test_load_kubeconfig_from_secret(self, mock_open, mock_base64, mock_client, mock_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("kubernetes_dashboard.kube_client.tempfile.gettempdir", return_value=temp_dir):
                result = load_kubeconfig_from_secret("test-secret", "test-namespace")
                
                # 함수 호출 확인
                mock_config.load_incluster_config.assert_called_once()
                mock_v1.read_namespaced_secret.assert_called_once_with("test-secret", "test-namespace")
